...
const tableDIR = "tablolar/amazon/amazon.xlsx";
...
keywords.map((k) => {
  ...
  urls.push({
    name: k,
    keywordSearch: `https://www.amazon.com.tr/s?k=${slugifyKeyword}&s=review-rank&ref=sr_st_review-rank`,
    links: [],
  });
});
...
(async () => {
  ...
  const openNextUrl = async () => {
    ...
    const linkElements = await page.$$(
      ".a-section.a-spacing-base a.a-link-normal.s-no-outline"
    );
  ...
  if (currentUrlIndex === urls.length) {
    for (const cat in urls) {
      ...
      const openKeywordUrl = async () => {
        ...
        const score = $(".a-size-medium.a-color-base").text();
        const color = $("#variation_color_name .selection").attr("title");
        const ebat = $("#variation_length_range .selection").text();
