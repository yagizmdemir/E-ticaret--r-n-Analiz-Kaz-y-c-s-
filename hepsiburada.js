...
const tableDIR = "tablolar/hepsiburada/hepsiburada.xlsx";
const productPerKeyword = 10;
...
keywords.map((k) => {
  const slugifyKeyword = k.replaceAll(" ", "%20");
  urls.push({
    name: k,
    keywordSearch: `https://www.hepsiburada.com/ara?q=${slugifyKeyword}&siralama=yorumsayisi`,
    links: [],
  });
});
...
(async () => {
  ...
  const openNextUrl = async () => {
    ...
    const linkElements = await page.$$(".productListContent-frGrtf5XrVXRwJ05HUfU.productListContent-rEYj2_8SETJUeqNhyzSm li a");
  ...
  if (currentUrlIndex === urls.length) {
    for (const cat in urls) {
      const tableHeaders = [
        ...
        { header: "Ürün Görsel Linki", key: "imageLink" },
      ];
      ...
      const openKeywordUrl = async () => {
        if (
          cat >= urls.length - 1 &&
          currentKeywordIndex >= urls[cat].links.length
        ) {
          console.log("\n\nTarama işlemi tamamlandı.");
          console.log(
            `Taranan ürünler ${tableDIR} excel tablosuna kaydedildi.`
          );
          await browser.close();
          process.exit();
        }
        ...
        const imageLink = $("#productDetailsCarousel > div.owl-stage-outer > div > div.owl-item.active > a > picture > img").attr("src");

        sheet.addRow({
          id: currentKeywordIndex + 1,
          url: url || "-",
          ad: ad.trim() || "-",
          price: price || "-",
          reviews: reviews || "-",
